...
function loadImgSeq() {
  ...
  dialog.showOpenDialog(dialog_options,function(imgseq_paths){
    ...
    $('.container-fluid:nth-child(2) .row:first-child').fadeOut(function(){
      ...
      $(".container-fluid:nth-child(2) .row:nth-child(2)").on("click","#detect_button",function(){detectSelectedImgSeq()})
...
function detectSelectedImgSeq() {
  ...
  state0.on('detectBatch',function(path,db){
    ...
    state1.on("detectImg",function(img){
      ...
      fpp.detect_(img_path,function(r){

        if(typeof(r) == "string") state1.emit('detectErr',img,r)

        if(typeof(r) == "object") state1.emit('detectSucc',img,r)
